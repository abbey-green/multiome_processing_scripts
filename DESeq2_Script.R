...
for (condition in conditions) {
    for (cell_type in cell_type_list) {
        ...
        rld_mat <- assay(rld)
        rld_cor <- cor(rld_mat)
        heatmap <- pheatmap(rld_cor, annotation = col_data[, c("condition"), drop=F],cluster_cols = FALSE, cluster_rows=FALSE)
        # Save the heatmap
        ggsave(paste(output_dir_2,'/',cell_type,"_",region,"_corr_heatmap.png", sep = ""), plot = heatmap)
        ...
        # MA Plot
        ma_plot <- plotMA(res)
        ggsave(paste(output_dir_2, "/",cell_type,"_",region,"_ma_plot.png", sep = ""), plot = ma_plot)
        
        # Heatmap of top differentially expressed genes
        topGenes <- rownames(res)[1:20]
        norm_counts <- assay(vst(dds))[topGenes, ]

        heatmap2 <- pheatmap(norm_counts, cluster_rows = TRUE, cluster_cols = TRUE, 
                 color = colorRampPalette(brewer.pal(9, "Blues"))(50))
        ggsave(paste(output_dir_2, "/",cell_type,"_",region,"_DEG_heatmap_plot.png", sep = ""), plot = heatmap2)
        }
        dev.off()
    }
